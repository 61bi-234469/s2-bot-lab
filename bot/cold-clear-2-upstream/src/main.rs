...
fn main() {
    ...
    let mut config: cold_clear_2_upstream::BotConfig = options.config.map_or_else(Default::default, |path| {
    ...
    futures::executor::block_on(cold_clear_2_upstream::run(incoming, outgoing, config));
}
