...
struct Driver { bot: Bot, stats: Statistics, limit: u64 }
...
enum Request {
    Start {
        ...
        #[serde(rename = "searchSelectionLimit")] search_selection_limit: String,
        #[serde(rename = "searchSeed")] search_seed: String,
...
pub unsafe extern "C" fn cc2_invoke(pointer: *const u8, length: usize) -> *mut u8 {
    ...
    let value = std::str::from_utf8(input).map_err(|_| "request-utf8")
        .and_then(|text| serde_json::from_str::<Request>(text).map_err(|_| "request-json"))
        .and_then(handle_request);
    encode(match value { Ok(value) => json!({ "ok": true, "value": value }), Err(error) => json!({ "ok": false, "error": error }) })
}
...
fn handle_request(request: Request) -> Result<Value, &'static str> {
    match request {
        ...
        Request::Suggest => DRIVER.with(|slot| {
            ...
            Ok(json!({ "moves": driver.bot.suggest(), "move_info": { "nodes": driver.stats.nodes, "selections": driver.stats.selections, "extra": "fixed selection budget complete" } }))
        }),
