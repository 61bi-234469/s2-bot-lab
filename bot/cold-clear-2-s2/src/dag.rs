...
impl<E: Evaluation> Dag<E> {
    ...
    pub fn suggest(&self, limit: usize) -> Vec<(Placement, f32)> {
        puffin::profile_function!();
    ...
    pub fn select<R: rand::Rng + ?Sized>(
        ...
        loop {
            ...
            match layer.kind.select(&game_state, speculate, exploration, rng) {
...
impl<E: Evaluation> WithBump<E> {
    ...
    fn suggest(&self, state: &GameState, limit: usize) -> Vec<(Placement, f32)> {
        puffin::profile_function!();
        self.with(|this| match this.data {
