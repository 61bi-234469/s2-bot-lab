...
import {
  S2_ALL_SPIN_MINI_EVALUATOR_ID,
  S2_ALL_SPIN_MINI_POLICY,
  evaluateS2AllSpinMini,
} from "./s2-all-spin-mini-evaluator.mjs";

export const S2_ALL_SPIN_MINI_SELECTOR_POLICY =
  "aligned-s2-score-plus-exact-all-spin-mini-minus-cc2-rank/1";

export function selectS2AllSpinMiniPlacement(guiState, moves, {
  candidateLimit = 16,
  rankPenalty = 25,
  adjustmentScale = 28,
  ...
  engineId = S2_ALL_SPIN_MINI_EVALUATOR_ID,
  comparisonSource = S2_ALL_SPIN_MINI_SELECTOR_POLICY,
  verificationMemo = null,
} = {}) {
  if (!Array.isArray(moves) || moves.length === 0 ||
    (!allowCompleteReturnedPrefix && moves.length < candidateLimit)) {
    throw new Error("All-Spin mini selector requires a complete CC2 candidate prefix");
  }
  if (!Number.isSafeInteger(candidateLimit) || candidateLimit < 1 || candidateLimit > 64 ||
    ...
    throw new Error("All-Spin mini selector has an invalid bounded setting");
  }
  ...
  const candidates = base.candidates.map(({ cc2Rank, identity, move, verification, placement }) => {
    ...
    const mini = evaluateS2AllSpinMini(base.state, record);
    const s2Score = scoreEvaluationFeatures(verification.comparison.features, model);
    return {
      cc2Rank, identity, move, verification, mini, s2Score,
      selectionScore: s2Score + adjustmentScale * mini.units - cc2Rank * rankPenalty,
    };
  });
  ...
  return {
    ...
    comparison: {
      ...
      moveSelectionPolicy: S2_ALL_SPIN_MINI_SELECTOR_POLICY,
      evaluatorPolicy: S2_ALL_SPIN_MINI_POLICY,
      candidateLimit,
