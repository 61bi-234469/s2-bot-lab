...
import {
  S2_MINI_TO_TSD_WITNESS_EVALUATOR_ID,
  S2_MINI_TO_TSD_WITNESS_POLICY,
  evaluateS2MiniToTsdWitness,
} from "./s2-mini-to-tsd-witness-evaluator.mjs";

export const S2_MINI_TO_TSD_WITNESS_SELECTOR_POLICY =
  "aligned-s2-score-plus-exact-mini-to-tsd-witness-minus-cc2-rank/1";

export function selectS2MiniToTsdWitnessPlacement(guiState, moves, {
  candidateLimit = 16,
  rankPenalty = 25,
  adjustmentScale = 28,
  ...
  engineId = S2_MINI_TO_TSD_WITNESS_EVALUATOR_ID,
  comparisonSource = S2_MINI_TO_TSD_WITNESS_SELECTOR_POLICY,
  verificationMemo = null,
} = {}) {
  if (!Array.isArray(moves) || moves.length === 0 ||
    (!allowCompleteReturnedPrefix && moves.length < candidateLimit)) {
    throw new Error("Mini-to-TSD selector requires a complete CC2 candidate prefix");
  }
  if (!Number.isSafeInteger(candidateLimit) || candidateLimit < 1 || candidateLimit > 64 ||
    ...
    throw new Error("Mini-to-TSD selector has an invalid bounded setting");
  }
  ...
  const candidates = base.candidates.map(({ cc2Rank, identity, move, verification, placement }) => {
    ...
    const witness = evaluateS2MiniToTsdWitness(base.state, record);
    const s2Score = scoreEvaluationFeatures(verification.comparison.features, model);
    return {
      cc2Rank, identity, move, verification, witness, s2Score,
      selectionScore: s2Score + adjustmentScale * witness.units - cc2Rank * rankPenalty,
    };
  });
  ...
  return {
    ...
    comparison: {
      ...
      moveSelectionPolicy: S2_MINI_TO_TSD_WITNESS_SELECTOR_POLICY,
      evaluatorPolicy: S2_MINI_TO_TSD_WITNESS_POLICY,
      candidateLimit,
