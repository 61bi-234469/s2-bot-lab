...
import {
  S2_REN_POST_TANK_DEFERRAL_EVALUATOR_ID,
  S2_REN_POST_TANK_DEFERRAL_POLICY,
  evaluateS2RenPostTankDeferral,
} from "./s2-ren-post-tank-deferral-evaluator.mjs";

export const S2_REN_POST_TANK_DEFERRAL_SELECTOR_POLICY =
  "aligned-s2-score-plus-exact-ren-post-tank-deferral-minus-cc2-rank/1";
export const S2_REN_POST_TANK_DEFERRAL_RETURNED_PREFIX_POLICY =
  "aligned-s2-score-plus-exact-ren-post-tank-deferral-returned-prefix-minus-cc2-rank/1";

/** Select only from the complete CC2 prefix; missing canonical evidence rejects the selection. */
export function selectS2RenPostTankDeferralPlacement(guiState, moves, {
  candidateLimit = 16,
  rankPenalty = 25,
  adjustmentScale = 28,
  ...
  engineId = S2_REN_POST_TANK_DEFERRAL_EVALUATOR_ID,
  comparisonSource = S2_REN_POST_TANK_DEFERRAL_SELECTOR_POLICY,
  verificationMemo = null,
} = {}) {
  if (!Array.isArray(moves) || moves.length === 0 ||
    (!allowCompleteReturnedPrefix && moves.length < candidateLimit)) {
    throw new Error("REN post-tank deferral selector requires a complete CC2 candidate prefix");
  }
  if (!Number.isSafeInteger(candidateLimit) || candidateLimit < 1 || candidateLimit > 64) {
    throw new Error("REN post-tank deferral candidateLimit must be an integer from 1 to 64");
  }
  if (!Number.isFinite(rankPenalty) || rankPenalty < 0 || rankPenalty > 100 ||
    ...
    throw new Error("REN post-tank deferral selector has an invalid score parameter");
  }
  if (typeof allowCompleteReturnedPrefix !== "boolean") {
    throw new Error("REN post-tank deferral allowCompleteReturnedPrefix must be boolean");
  }
  const base = buildCompleteCc2FinalPlacementCandidates(guiState, moves, {
    ...
    comparisonSource,
    verificationMemo,
    selectorLabel: "REN post-tank deferral",
  });
  ...
  const candidates = base.candidates.map(({ cc2Rank, identity, move, verification, placement }) => {
    ...
    const deferral = evaluateS2RenPostTankDeferral(base.state, record);
    const s2Score = scoreEvaluationFeatures(verification.comparison.features, model);
    return { cc2Rank, identity, move, verification, deferral, s2Score,
      selectionScore: s2Score + adjustmentScale * deferral.units - cc2Rank * rankPenalty };
  });
  ...
  return {
    ...
    comparison: {
      ...
      moveSelectionPolicy: allowCompleteReturnedPrefix
        ? S2_REN_POST_TANK_DEFERRAL_RETURNED_PREFIX_POLICY
        : S2_REN_POST_TANK_DEFERRAL_SELECTOR_POLICY,
      candidateLimit,
      ...
      renPostTankDeferral: best.deferral,
      adjustmentScale,
    },
