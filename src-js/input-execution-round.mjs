...
export function createInputExecutionRound({ seed, ids = ['left', 'right'], handlingById = {},
  ...
  const sessions = ids.map(id => createInputReplaySession({ id, replay: {
    frames: 0, events: [], options: inputExecutionOptions({ seed, handling: handlingById[id], timeProgression }), results: { stats: { garbage: { sent: 0 } } },
  ...
  const round = {
    ...
    plan(id, { moves, type = 'cc2-s2-f14', engineId = type }, budget) {
      ...
      return resolveQualifiedInputSubmission(request, movement, { timeProgression, ...budget });
    },
