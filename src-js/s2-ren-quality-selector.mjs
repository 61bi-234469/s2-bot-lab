...
import {
  S2_REN_QUALITY_EVALUATOR_ID, S2_REN_QUALITY_POLICY, evaluateS2RenQuality,
} from "./s2-ren-quality-evaluator.mjs";

export const S2_REN_QUALITY_SELECTOR_POLICY =
  "aligned-s2-plus-exclusive-ren-quality-minus-cc2-rank/1";

export function selectS2RenQualityPlacement(guiState, moves, {
  candidateLimit = 16, rankPenalty = 25, adjustmentScale = 28,
  weightProfileId = "sparse-s2", weights = {}, allowCompleteReturnedPrefix = false,
  engineId = S2_REN_QUALITY_EVALUATOR_ID,
  comparisonSource = S2_REN_QUALITY_SELECTOR_POLICY,
  verificationMemo = null,
} = {}) {
  if (!Array.isArray(moves) || moves.length === 0 ||
      (!allowCompleteReturnedPrefix && moves.length < candidateLimit)) {
    throw new Error("REN quality selector requires a complete CC2 candidate prefix");
  }
  if (!Number.isSafeInteger(candidateLimit) || candidateLimit < 1 || candidateLimit > 64 ||
    ...
    throw new Error("REN quality selector has an invalid bounded setting");
  }
  ...
  const candidates = base.candidates.map(({ cc2Rank, identity, move, verification, placement }) => {
    ...
    const renQuality = evaluateS2RenQuality(base.state, record);
    const s2Score = scoreEvaluationFeatures(verification.comparison.features, model);
    return { cc2Rank, identity, move, verification, renQuality, s2Score,
      selectionScore: s2Score + adjustmentScale * renQuality.units - cc2Rank * rankPenalty };
  });
  ...
  return {
    ...
    comparison: {
      ...
      moveSelectionPolicy: S2_REN_QUALITY_SELECTOR_POLICY,
      evaluatorPolicy: S2_REN_QUALITY_POLICY,
    ...
    candidates: candidates.map((candidate) => ({
      ...
      renQuality: candidate.renQuality,
    })),
  };
}
