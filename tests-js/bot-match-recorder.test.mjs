...
function submissionsFor(match) {
  return match.bots.map((bot) => {
    const best = analyzeSimpleS2FinalPlacements(bot.state, { topN: 1 }).moves[0];
    return {
      botId: bot.id,
      result: {
        transition: best.transition,
        comparison: {
          positionFingerprint: fullStateKey(bot.state),
          witness: { placement: best.placement },
        },
      },
    };
...
test("appendMatchLocks preserves immutable recorder results while mutating the owned recording", () => {
  const { after: before, recording } = setup();
  const submissions = submissionsFor(before);
  const after = advanceBotMatch(before, submissions);
  const original = structuredClone(recording);
  const expected = recordMatchLocks(recording, before, after, submissions);
  const owned = structuredClone(recording);

  assert.strictEqual(appendMatchLocks(owned, before, after, submissions), owned);
  assert.deepEqual(owned, expected);
  assert.deepEqual(recording, original, "immutable API input must remain unchanged");
});

test("appendMatchLocks stages every lock before committing a failure", () => {
  const { after: before, recording } = setup();
  const submissions = submissionsFor(before);
  const after = advanceBotMatch(before, submissions);
  const invalid = {
    ...submissions[1],
    result: { ...submissions[1].result, transition: null },
  };
  const snapshot = structuredClone(recording);

  assert.throws(
    () => appendMatchLocks(recording, before, after, [submissions[0], invalid]),
    /has no next state/,
  );
  assert.deepEqual(recording, snapshot);
});

test("a finished round stays fixed after its owned recorder receives another append", () => {
  const { after: before, recording } = setup();
  ...
  appendMatchLocks(recording, before, after, submissions);
